...
public class StreamDemoKafka2Mysql {
    public static void main(String[] args) throws Exception {
        ...
        //设置模式为：exactly_one，仅一次语义，默认就是EXACTLY_ONCE
        ...
        env.setStateBackend((StateBackend) new FsStateBackend("file:/data/flinkdata/checkpoint"));
...
//        FlinkKafkaConsumer011<ObjectNode> kafkaConsumer011 = new FlinkKafkaConsumer011<>("wangyntest03", new JSONKeyValueDeserializationSchema(true), props);
        ...
        DataStream<Tuple2<String, Integer>> wordcount = kafkaStreamSource.flatMap(
                new FlatMapIterator<String, Tuple2<String, Integer>>() {
            @Override
            public Iterator<Tuple2<String, Integer>> flatMap(String s) throws Exception {
                ArrayList<Tuple2<String, Integer>> list = new ArrayList();
                String[] tokens = s.split(" ");
                for(String token:tokens){
                    list.add(new Tuple2<String,Integer>(token,1));
                }
                return list.iterator();
            }

        }).keyBy(0).sum(1);
        wordcount.writeAsText(outputPath, FileSystem.WriteMode.OVERWRITE).name(topic + "..." + outputPath);
